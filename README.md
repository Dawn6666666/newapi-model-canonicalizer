...
浏览器端 Userscript：用于为 New-API 渠道生成“标准模型 canonical key -> 渠道真实模型名”的 `model_mapping`，并通过约束避免别名互映/回环。
...
- 提供仪表盘（about:blank）用于审阅 Diff/Plan/DB。

安装与使用
1. 安装 Tampermonkey/Violentmonkey
2. 安装 `userscript/newapi-model-canonicalizer.user.js`
